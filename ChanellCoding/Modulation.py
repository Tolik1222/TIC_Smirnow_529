...
def ask_demodulation(frequency, sequence):
    ...
    for i in range(0, 10):
        ...
        for t in range(0, 100):
            S = S + ask_product[t + 100 * (i - 1)] if i > 0 else ask_product[t]
            ask_demodulated_signal[t + 100 * (i - 1)] = S if i > 0 else ask_product[t]
    ask_demodulated = 1 / 2 * (np.sign(ask_demodulated_signal - threshold) + 1)
    for i in range(0, 10):
        for t in range(0, 100):
            sequence_demodulated[t + 100 * (i - 1)] = ask_demodulated[100 * i - 1] if i > 0 else ask_demodulated[0]
...
def psk_demodulation(frequency, sequence):
    ...
    for i in range(0, 10):
        S = 0
        for t in range(0, 100):
            S = S + psk_product[t + 100 * (i - 1)] if i > 0 else psk_product[t]
            psk_demodulated_signal[t + 100 * (i - 1)] = S if i > 0 else psk_product[t]
    psk_demodulated = 1 / 2 * (np.sign(psk_demodulated_signal - threshold) + 1)
    for i in range(0, 10):
        for t in range(0, 100):
            sequence_demodulated[t + 100 * (i - 1)] = psk_demodulated[100 * i - 1] if i > 0 else psk_demodulated[0]
...
def fsk_demodulation(frequency1, frequency2, sequence):
    ...
    for i in range(0, 10):
        ...
        S2 = 0
        for t in range(0, 100):
            S1 = S1 + fsk_product1[t + 100 * (i - 1)] if i > 0 else fsk_product1[t]
            fsk_demodulated_signal1[t + 100 * (i - 1)] = S1 if i > 0 else fsk_product1[t]
            S2 = S2 + fsk_product2[t + 100 * (i - 1)] if i > 0 else fsk_product2[t]
            fsk_demodulated_signal2[t + 100 * (i - 1)] = S2 if i > 0 else fsk_product2[t]
...
def noise_stress_test(sequence, sequence_modulated, modulation, frequency):
    error_modulated = []
    sequence_demodulated = np.zeros(1000)
    for i in range(0, 20):
        p = 0
        for m in range(0, 200):
            ...
            if modulation == "ASK":
                ...
                fsk_demodulated_signal, fsk_demodulated_signal2, sequence_demodulated = fsk_demodulation(frequency[0], frequency[1], sequence_noise)
...
def main(ask, psk, fsk1, fsk2):
    ...
    plot(x_spectrum, spectrum_sequence_fsk, axis_x="Частота, Гц", axis_y="Амплітуда спектру", title="Спектр при частотній модуляції")
    ...
    error_ask = noise_stress_test(sequence, sequence_ask, "ASK", [ask])
    error_psk = noise_stress_test(sequence, sequence_psk, "PSK", [psk])
    error_fsk = noise_stress_test(sequence, sequence_fsk, "FSK", [fsk1, fsk2])
    fig, ax = plt.subplots(figsize=(21 / 2.54, 14 / 2.54))
